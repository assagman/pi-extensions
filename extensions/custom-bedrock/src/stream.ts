...
function convertToolConfig(tools: Tool[] | undefined): ToolConfiguration | undefined {
...
function mapStopReason(reason: string | undefined): "length" | "stop" | "toolUse" {
  switch (reason) {
...
export function streamCustomBedrock(
  model: Model<Api>,
  context: Context,
  ...
  (async () => {
    ...
    try {
      const config: BedrockRuntimeClientConfig = {
        ...
        signer: async () => ({
          sign: async <T>(request: T): Promise<T> => request,
        }),
      };
      ...
      const command = new ConverseStreamCommand({
        ...
        additionalModelRequestFields: additionalFields as never,
      });
